...
// ùùùùùùùùùùùùùùùùùùù  ùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùùù
...
// cioù se c'ù spazio vuoto a destra
METHOD _THISXBP_NAME:isFitColumns()
   ...
   // se in origine c'ù spazio vuoto a destra abilito il riempimento
   ...
   // se c'ù spazio vuoto a destra abilito il riempimento
...
METHOD S2XbpBrowser:Create( oParent, oOwner, aPos, aSize, aPresParam, lVisible )
   ...
   IF ::Status() == XBP_STAT_INIT
      ...
      // Ricalcolo larghezza colonne perchù se ce n'ù una con :width < 5
      // non funziona
...
//    // Se ho selezionato e c'ù una colonna di TAG e ho il cursore
...
METHOD S2XbpBrowser:configureColumns()
   ...
   FOR nInd := 1 TO ::colCount
      ...
      // Lo faccio sempre perchù con il font TAHOMA
...
// Commentato perchù fa dei FLASH terribili
// #ifdef _XBASE15_
//    ::nDispLoop++
//
//    IF ::nDispLoop==1
...
METHOD S2XbpBrowser:__tbInsColumn(nColPos, bBlock, nWidth, cId, cPrompt, ;
   ...
   // corrente non viene trovata perche ancora non ù aggiunta
...
METHOD S2XbpBrowser:dfTAGTotalInc()     // Inc dei Totali  Colonne Tag
   ...
   FOR nN := 1 TO LEN(::aTotal)
       oSub:= ::aTotal[nN]
       IF oSub:CountMode==COLUMN_TAG_COUNT
          // Solo WC_TOTALVALUE: EVAL(BLOCK) duplicava lavoro (lookup/PG) per riga
...
METHOD S2XbpBrowser:dfTotalInc()     // Inc dei Totali
   // tbEval posiziona giù il record; WC_TOTALVALUE legge i campi correnti.
...
// puù essere chiamato solo dopo il :create()
