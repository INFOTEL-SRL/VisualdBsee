...
FOR nIndex := 1 TO LEN(aNtxTag)
// simone 10/4/08 commentato perch� non serve
...
WHILE UPPER(dbdd->recTyp)=="DBF" .AND. !dbdd->(EOF())
         ...
         ELSE
            ...
            //IF nHnd<0 .AND. !dfFtDriver( cDriver )
               ...
               IF nMode == IDX_CHECK .AND. ;
                  ...
                  // di aprire tutti gli altri perch� su windows NT
                  // � un'operazione LENTA!
                  lSkipOpen := .T.
               ENDIF
            ELSE
   ...
   WHILE UPPER(dBdd->RecTyp+dBdd->file_name)=="NDX" +UPPER(cName) .AND. !DBDD->(EOF())
      ...
      AADD( aNtxDat, { dBdd->(RECNO())       ,; //  1 N�record dbdd
 ...
 ALLTRIM(UPPER(ddGetSlot(dbdd->Slot,"�",3)))=="U" ,;  // 11 Indice di tipo UNIQUE
      ...
      IF nMode==IDX_CHOICE    // Ricostruzione parziale con selezione
         AADD( aNtxLst, cOpen  +" "                 +; // 1 in rete Lock
                        ...
                        STR(nRecNum,6)         +" " +; // 3 N� Records del file
...
// DBFNTX allora controllo se il DBF � vuoto e ha campi memo
// ne creo uno vuoto al volo usando ADS
//
// NOTA: non � STATIC FUNCTION perche cosi si pu� utilizzare
...
// Spostato sopra perche in caso di ADS cDriver � un oggetto e da runtime errore quando si cerca di di fare un <Alltrim()>
...
//      cDriver := dfAXSDriver()
   ...
   IF UPPER(ALLTRIM(cDriver)) == dfAXSDriver() .AND. ;
      ...
      BEGIN SEQUENCE
         ...
         // a questo punto il DBF � chiuso, se ha zero record e
